...
"""
workstation/roles/desktop/tasks/local.yml
"""
...
def test_deploy_konsolerc_config__file_owner(host):
    """Verify file owner"""
    assert (
        host.file(f"/home/{USER['username']}/.config/konsolerc").user
        == USER["username"]
    )
...
def test_deploy_konsolerc_config__file_group(host):
    """Verify file group"""
    assert (
        host.file(f"/home/{USER['username']}/.config/konsolerc").group
        == USER["username"]
    )
...
def test_create_konsole_config_dir__file_owner(host):
    ...
    assert (
        host.file(f"/home/{USER['username']}/.local/share/konsole").user
        == USER["username"]
    )
...
def test_create_konsole_config_dir__file_group(host):
    ...
    assert (
        host.file(f"/home/{USER['username']}/.local/share/konsole").group
        == USER["username"]
    )
...
def test_deploy_konsole_profile__file_owner(host):
    """Verify file owner"""
    assert (
        host.file(f"/home/{USER['username']}/.local/share/konsole/green.profile").user
        == USER["username"]
    )
...
def test_deploy_konsole_profile__file_group(host):
    """Verify file group"""
    assert (
        host.file(f"/home/{USER['username']}/.local/share/konsole/green.profile").group
        == USER["username"]
    )
...
def test_deploy_zshrc__file_owner(host):
    """Verify file owner"""
    assert host.file(f"/home/{USER['username']}/.zshrc").user == USER["username"]


def test_deploy_zshrc__file_group(host):
    """Verify file group"""
    assert host.file(f"/home/{USER['username']}/.zshrc").group == USER["username"]


def test_deploy_zshrc__file_contents(host):
    """Verify file contents"""
    assert host.file(f"/home/{USER['username']}/.zshrc").content_string == read_file(
        f"{PLAYPATH}/files/zsh/zshrc"
    )
...
def test_create_zsh_home_dir__owner(host):
    """Verify directory owner"""
    assert host.file(f"/home/{USER['username']}/.zsh_home").user == USER["username"]


def test_create_zsh_home_dir__group(host):
    """Verify directory group"""
    assert host.file(f"/home/{USER['username']}/.zsh_home").group == USER["username"]
...
def test_deploy_home_zsh__file_owner(host):
    """Verify file owner"""
    assert (
        host.file(f"/home/{USER['username']}/.zsh_home/home.zsh").user
        == USER["username"]
    )
...
def test_deploy_home_zsh__file_group(host):
    """Verify file group"""
    assert (
        host.file(f"/home/{USER['username']}/.zsh_home/home.zsh").group
        == USER["username"]
    )
